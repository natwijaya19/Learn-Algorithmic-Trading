...
stock_symbol = "GOOG"
try:
    goog_data2: pd.DataFrame = pd.read_pickle(SRC_DATA_FILENAME)
except FileNotFoundError:
    # goog_data2 = df = pdr.get_data_yahoo(stock_symbol, start_date, end_date)
    goog_data2: pd.DataFrame = yf.download(stock_symbol, start=start_date, end=end_date)
    goog_data2.to_pickle(SRC_DATA_FILENAME)

goog_data: DataFrame = goog_data2.tail(620)

close: Series = goog_data["Close"]
