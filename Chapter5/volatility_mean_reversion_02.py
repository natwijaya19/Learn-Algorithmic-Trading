...
for close_price in close:
    ...
    if (
        len(price_history) > SMA_NUM_PERIODS
    ):  # we track at most 'time_period' number of prices
        del price_history[0]
    ...
    if (
        apo > APO_VALUE_FOR_SELL_ENTRY * stdev_factor
        and abs(close_price - last_sell_price)
    ...
    # 2. We are short( -ve position ) and either APO trading signal value is at or below 0 or current position is profitable enough to lock profit.
        ...
        apo < APO_VALUE_FOR_BUY_ENTRY * stdev_factor
        and abs(close_price - last_buy_price)
        > MIN_PRICE_MOVE_FROM_LAST_TRADE * stdev_factor
