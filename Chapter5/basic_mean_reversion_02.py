...
last_buy_price: int = 0  # Price at which last buy trade was made, used to prevent over-trading at/around the same price
...
close_price: float = 0  # Initialize variable to hold closing price
for close_price in close:
    ...
    # different enough.
    # 2. We are long( +ve position ) and either APO trading
    # signal value is at or above 0 or current position is profitable enough
    # to lock profit.
    if (
    ...
    ):  # long from -ve APO and APO has gone positive or position is
    ...
    # and the difference between last trade-price and current-price is
    # different enough.
    # 2. We are short( -ve position ) and either APO
    # trading signal value is at or below 0 or current position is profitable
    ...
    if position > 0:
        if (
            ...
            # close that amount based on how much was sold against this long
            # position
...
for i in range(
    APO_VALUE_FOR_BUY_ENTRY, APO_VALUE_FOR_BUY_ENTRY * 5, APO_VALUE_FOR_BUY_ENTRY
):
...
plt.axhline(y=0, lw=0.5, color="k")
for i in range(
    NUM_SHARES_PER_TRADE, NUM_SHARES_PER_TRADE * 25, NUM_SHARES_PER_TRADE * 5
):
    plt.axhline(y=i, lw=0.5, color="r")
for i in range(
    -NUM_SHARES_PER_TRADE, -NUM_SHARES_PER_TRADE * 25, -NUM_SHARES_PER_TRADE * 5
...
plt.plot(
    data.loc[data.Pnl > 0].index, data.Pnl[data.Pnl > 0], color="g", lw=0, marker="."
)
plt.plot(
    data.loc[data.Pnl < 0].index, data.Pnl[data.Pnl < 0], color="r", lw=0, marker="."
)
plt.legend()
plt.show()
