...
class OMS:
    def __init__(self, sim_real_clock):
        self.sim_real_clock = sim_real_clock
        self.five_sec_order_time_out_management = TimeOut(
            sim_real_clock, sim_real_clock.getTime() + timedelta(0, 5), self.onTimeOut
        )
...
if __name__ == "__main__":
    ...
    simulated_real_clock.process_order(
        {"id": 1, "timestamp": "2018-06-29 08:15:27.243860"}
    )
    oms = OMS(simulated_real_clock)
    oms.send_order()
    simulated_real_clock.process_order(
        {"id": 1, "timestamp": "2018-06-29 08:21:27.243860"}
    )
