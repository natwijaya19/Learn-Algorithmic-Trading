...
class OrderBook:
    ...
    def __init__(self, gt_2_ob: deque = None, ob_to_ts: deque = None):
        self.list_asks = []
        self.list_bids = []
    ...
    def handle_order_from_gateway(self, order: dict[str, str | int] = None):
        if self.gw_2_ob is None:
            print("simulation mode")
            self.handle_order(order)
        elif len(self.gw_2_ob) > 0:
            order_from_gw: dict[str, str | int] = self.gw_2_ob.popleft()
            self.handle_order(order_from_gw)

    def handle_order(self, order: dict[str, str | int]):
        if order["action"] == "new":
